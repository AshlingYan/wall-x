...
class ActionQueue:
    ...
    def merge(self, new_actions: np.ndarray, execute_actions: np.ndarray, inference_delay: int):
        ...
        for ep in range(args.num_episode):
            ...
            while run_condition(step):
                ...
                if action_queue.size() == 0:
                    ...
                    action_queue.merge(action_chunk, execute_actions, rtc_config.inference_delay if rtc_config.enabled else 0)

                    # Print RTC info if enabled
                    if rtc_config.enabled:
                        stats = action_queue.get_stats()
                        print(f"   ✅ Queued {len(execute_actions)} action(s) | RTC: blend={stats['has_leftover']} | queue={stats['queue_size']}")
                    else:
                ...
                if action_queue.size() > 0:
                    ...
                    if a is not None:
                        ...
                        print(f"   🎯 Executing action (mode: {action_mode}), remaining queue: {action_queue.size()}")
