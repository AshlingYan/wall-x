...
class WallXServer:
    def __init__(self, checkpoint_path, device="cuda:0", control_freq=10):
        ...
        # self.model = self.model.bfloat16()  # Disable to avoid dtype mismatch
    ...
    def prepare_inputs(self, images, state, instruction="Stack the blocks"):
        ...
        # Assistant response
        action_chunk_size = 10  # Flow action horizon
        ...
        position_ids = torch.arange(seq_length, dtype=torch.long).unsqueeze(0).unsqueeze(1).expand(batch_size, 1, -1)

        agent_pos_mask = torch.ones((batch_size, 1, state_dim), dtype=torch.float32)
        dof_mask = torch.ones((batch_size, 10, state_dim), dtype=torch.float32)
        ...
        agent_pos_mask = agent_pos_mask.to(device)
    ...
    def run_inference(self, model_inputs):
        ...
        with torch.no_grad():
            try:
                ...
                elif isinstance(output, dict) and "predict_action" in output:
                    action = output["predict_action"]
                    if isinstance(action, torch.Tensor):
                        action = action.cpu().float().numpy()
                    # 取第一帧的动作 (action_horizon, action_dim) -> (action_dim,)
    ...
    def denormalize_action(self, action, checkpoint_path, dataset_name="stack_blocks_test_v1"):
        """反归一化"""
        normalizer_path = os.path.join(checkpoint_path, "normalizer_action.pth")
        if os.path.exists(normalizer_path):
            from wall_x.model.action_head import Normalizer
            normalizer = Normalizer.from_ckpt(normalizer_path)
    ...
    def infer(self, message):
        ...
        debug_print("Server", f"Message keys: {message.keys()}", "DEBUG")
        debug_print("Server", f"Number of images: {len(message.get('img_arr', []))}", "DEBUG")
...
def main():
    ...
    try:
        while True:
            ...
            # 等待接收线程启动
            time.sleep(0.2)
            debug_print("Server", "Receiver thread ready, waiting for messages...", "INFO")
