...
class QwenVlAct_Trainer:
    ...
    def load_normalizer(self):
        import copy
        # Start with default action statistics
        action_statistic_dof = copy.deepcopy(default_action_statistic_dof)
        
        # Update with customized robot config if available
        if self.config.get("enable_customized_robot_config", False):
            norm_stats_path = self.config.get("norm_stats_path", None)
            if norm_stats_path:
                robot_name = self.config.get("customized_robot_config", {}).get("name", None)
                if robot_name:
                    self.print_rank0(f"Updating action statistics for {robot_name} from {norm_stats_path}")
                    update_action_statistics(
                        ...
                        customized_dof_config=self.config.get("customized_robot_config", {}).get("customized_dof_config", None),
                        customized_agent_pos_config=self.config.get("customized_robot_config", {}).get("customized_agent_pos_config", None),
                    )

        # Remove the old if-else block since we handle it above now
