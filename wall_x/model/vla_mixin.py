...
try:
    ...
    # AttentionInterface not available in transformers < 5.0, create a stub
    class AttentionInterface:
        def valid_keys(self):
            return ["sdpa", "eager", "flash_attention_2"]
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            if len(self.action_token_id_set.get("action_token_list", self.action_token_id_set.get("fast_action_token_list", []))) > 0:
                shift_logits = logits[..., :-1, :].contiguous()
                action_preds = shift_logits.argmax(dim=-1)
                shift_labels = labels[..., 1:].contiguous()
                _action_token_list = self.action_token_id_set.get("action_token_list", self.action_token_id_set.get("fast_action_token_list", []))
                action_mask = (
                    shift_labels > _action_token_list[0]
                )
