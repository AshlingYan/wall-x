...
class Qwen2_5_VLMoEForAction(
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # Filter out state_dict keys with shape mismatch (e.g., action head dim change)
        model_state = model.state_dict()
        mismatched_keys = []
        for key in list(state_dict.keys()):
            if key in model_state and state_dict[key].shape != model_state[key].shape:
                print(f"filter shape-mismatched weight {key}: "
                      f"checkpoint {state_dict[key].shape} vs model {model_state[key].shape}")
