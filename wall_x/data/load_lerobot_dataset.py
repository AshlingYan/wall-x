    ...
    def _vision_preprocess(self, frames):
        ...
        for key in self.hf_dataset.meta.camera_keys:
            ...
            if target_size != -1:
                ...
                # Set resized dimensions for return value
                resized_height, resized_width = new_height, new_width
                # Skip smart_resize when target_size is configured
                processed_frames.append(img_pil)
                continue

            # 3. Apply smart scaling (qwen logic) - only if no target_size
