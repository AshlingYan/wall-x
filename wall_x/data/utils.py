...
def load_norm_stats(norm_stats_path, dataset_name):
    ...
    # Handle cases where the key in norm_stats differs from KEY_MAPPINGS
    # Try action_key first, fall back to 'actions' if not found
    if action_key in norm_stats["norm_stats"]:
        action_stats_key = action_key
    else:
        action_stats_key = "actions"
    
    # Try state_key first, fall back to 'state' if not found  
    if state_key in norm_stats["norm_stats"]:
        state_stats_key = state_key
    else:
        state_stats_key = "state"
    
    q01 = torch.tensor(norm_stats["norm_stats"][action_stats_key]["q01"])
    q99 = torch.tensor(norm_stats["norm_stats"][action_stats_key]["q99"])
    delta = q99 - q01
    ...
    q01 = torch.tensor(norm_stats["norm_stats"][state_stats_key]["q01"])
    q99 = torch.tensor(norm_stats["norm_stats"][state_stats_key]["q99"])
    delta = q99 - q01
